...
function renderFiles(data) {
  ...
  if (files.length === 0) {
    list.innerHTML = '<div class="empty">No files yet 鈥?drag something in!</div>';
    return;
  }
  ...
  list.innerHTML = files.map(f => {
    ...
    const downloadBtn = `<button class="btn btn-sm" onclick="downloadFile('${f.name}')" title="Download">猬?/button>`;
    const copyBtn = `<button class="btn btn-sm" onclick="copyLink('${f.name}')" title="Copy link">馃敆</button>`;
    const delBtn = `<button class="btn btn-sm btn-danger" onclick="deleteFile('${f.name}')" title="Delete">馃棏</button>`;
...
function updateStats(data) {
  ...
  el.textContent = `Uptime: ${uptime}  |  鈫?${data.upload_count || 0}  |  鈫?${data.download_count || 0}  |  馃棏 ${data.delete_count || 0}`;
}
...
async function uploadFiles(files) {
  ...
  try {
    ...
    for (const item of items) {
      item.statusEl.textContent = '鉁?done';
      item.statusEl.className = 'status ok';
    }
  } catch (err) {
    for (const item of items) {
      item.statusEl.textContent = '鉁?failed';
